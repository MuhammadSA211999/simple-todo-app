...
import { selectTodoColor, toggledTodo, deletedTodo } from "../redux/todo/action";

export default function Todo({ todo }) {
    const { completed, color, id, text } = todo
    const dispatch = useDispatch()
    const handleStatusChanged = (todoId) => {
        dispatch(toggledTodo(todoId))
    }
    const handleColorChanged = (id, color) => {
        dispatch(selectTodoColor(id, color))
    }
    const handleDeleteTodo = (id) => {
        dispatch(deletedTodo(id))
    }

    const tickClass = `border-green-500 focus-within:border-green-500`
    return (
        <div className="flex justify-start items-center p-2 hover:bg-gray-100 hover:transition-all space-x-4 border-b border-gray-400/20 last:border-0">
            <div className={`rounded-full bg-white border-2 border-gray-400 w-5 h-5 flex flex-shrink-0 justify-center items-center mr-2 ${completed ? tickClass : ''}`}>
                <input
            ...
            <div onClick={() => handleColorChanged(id, 'green')}
            ...
            <div onClick={() => handleColorChanged(id, 'yellow')}
            ...
            <div onClick={() => handleColorChanged(id, 'red')}
            ...
            <img onClick={() => handleDeleteTodo(id)}
                src={cancelImage}
